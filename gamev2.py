...
stuff = ['plate', 'bottle', 'cup', 'monitor', 'bomb']    #entities in the game
CONVRAT = 1.0 * pow(10, -6)
elapsedTime = 0
a = 0
b = 0
...
def generate_random_stuff(fruit):
    fruit_path = "images/" + fruit + ".png"
    data[fruit] = {
        'img': pygame.image.load(fruit_path),
        'x' : random.randint(100,WIDTH),          #where the fruit should be positioned on x-coordinate
        'y' : HEIGHT*1.2,
        'speed_x': random.randint(-10,10),      #how fast the fruit should move in x direction. Controls the diagonal movement of stuff
        'speed_y': random.randint(-80, -60),    #control the speed of stuff in y-directionn ( UP )
        'throw': False,                         #determines if the generated coordinate of the stuff is outside the gameDisplay or not. If outside, then it will be discarded
        't': 0,                                 #manages the
        'hit': False,
    }

    if random.random() >= 0.75:     #Return the next random floating point number in the range [0.0, 1.0) to keep the stuff inside the gameDisplay
        data[fruit]['throw'] = True
    else:
        data[fruit]['throw'] = False

# Dictionary to hold the data the random stuff generation
data = {}
for fruit in stuff:
    generate_random_stuff(fruit)
...
def updateMouse():
    mouse_x = pyautogui.position()[0] + scaleDist(x_val)
    mouse_y = pyautogui.position()[1] + scaleDist(z_val)
...
while game_running :
    ...
    for key, value in data.items():
        if value['throw']:
            value['x'] += value['speed_x']          #moving the stuff in x-coordinates
            value['y'] += value['speed_y']          #moving the stuff in y-coordinate
            ...
            if not value['hit'] and current_position[0] > value['x'] and current_position[0] < value['x']+60 \
                ...
                if key == 'bomb':
                    ...
                    half_fruit_path = "images/" + "broken_" + key + ".png"
